...
function displayArea(coordinates, name, eng_name) {
    ...
    kakao.maps.event.addListener(polygon, 'click', function () {
    	...
    	fetch("congestion/region", {
    	    method: "POST",
    	    headers: {
    	        "Content-Type": "application/json",
    	    },
    	    body: JSON.stringify({
    	        month: document.getElementById('month').value,  // 현재 선택된 연,월을 json 데이터로 전달 
    	        region: eng_name,  // 클릭한 지역의 영어 이름을 json 데이터로 전달
    	    }),
    	})
    	.then(response => response.text())
    	.then(data => {
    	    // 서블릿으로부터 받은 응답을 처리
    	    const valuesArray = data.split(","); // response로 받은 데이터(String)을 "," 로 split하기
    	    const congestionValue = valuesArray[0]; // 나눈 data 중 0번째 data(congestionValue);
    	    const congestion = valuesArray[1]; // 나눈 data 중 1번째 data(congestion);
    	    ...
    	    console.log("검색지역 : " + valuesArray[2]);
    	    console.log("검색 시점 : " + valuesArray[3] + "년, " + valuesArray[4] + "월");
